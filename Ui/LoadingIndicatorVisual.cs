...
public sealed class LoadingIndicatorVisual : FrameworkElement, IDisposable
{
    private const int PointCount = 180;
    private const double ShapeDurationMilliseconds = 650;
    private const double ShapeDiameterRatio = 0.632;
    private const double ConstantRotationDegrees = 50;
    private const double ExtraRotationDegrees = 90;
    private const double SpringStiffness = 200;
    private const double SpringDampingRatio = 0.6;
    private static readonly Lazy<IReadOnlyList<Point[]>> Shapes = new(CreateShapes);

    public static readonly DependencyProperty FillProperty = DependencyProperty.Register(
        nameof(Fill),
        typeof(Brush),
        typeof(LoadingIndicatorVisual),
        new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.AffectsRender));

    private readonly Stopwatch _clock = new();
    private TimeSpan _previousRenderTime;
    private double _elapsedMilliseconds;
    private double _springPosition;
    private double _springVelocity;
    private double _morphTarget = 1;
    private int _previousCycle;
    private double _rotation;
    private bool _rendering;

    public LoadingIndicatorVisual()
    {
        Width = 56;
        Height = 56;
    ...
    internal bool IsRequestedActive { get; private set; }

    public void Start(bool animationsEnabled)
    {
        ...
        {
            _springPosition = Shapes.Value.Count - 1;
            InvalidateVisual();
            return;
        }
    ...
    public void BeginStop() => IsRequestedActive = false;

    public void Stop()
    {
        IsRequestedActive = false;
        if (!_rendering) return;
        _rendering = false;
        CompositionTarget.Rendering -= OnRendering;
        _clock.Stop();
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        var scale = Math.Min(width, height) * ShapeDiameterRatio / 2;
        var points = MorphedShape(_springPosition);
        ...
        drawingContext.PushTransform(new RotateTransform(_rotation, center.X, center.Y));
    ...
    private void OnRendering(object? sender, EventArgs e)
    {
        ...
        var deltaSeconds = Math.Min((renderTime - _previousRenderTime).TotalSeconds, 0.1);
        _previousRenderTime = renderTime;
        if (deltaSeconds <= 0) return;

        _elapsedMilliseconds += deltaSeconds * 1000;
        var cycle = (int)Math.Floor(_elapsedMilliseconds / ShapeDurationMilliseconds);
        ...
        var fraction = _elapsedMilliseconds % ShapeDurationMilliseconds / ShapeDurationMilliseconds;
        var baseShape = _morphTarget - 1;
        var perShapeMorph = _springPosition - baseShape;
        _rotation = ((ConstantRotationDegrees + ExtraRotationDegrees) * baseShape
            + ConstantRotationDegrees * fraction
            + ExtraRotationDegrees * perShapeMorph) % 360;
        InvalidateVisual();
    }
    ...
    private void StepSpring(double deltaSeconds)
    {
        var damping = SpringDampingRatio * 2 * Math.Sqrt(SpringStiffness);
        var substep = deltaSeconds / 12;
        for (var index = 0; index < 12; index++)
        {
            var acceleration = -SpringStiffness * (_springPosition - _morphTarget)
                - damping * _springVelocity;
            _springVelocity += acceleration * substep;
            _springPosition += _springVelocity * substep;
        }
    }

    private static Point[] MorphedShape(double fraction)
    {
        var shapes = Shapes.Value;
        var index = (int)Math.Floor(fraction);
        var from = ((index % shapes.Count) + shapes.Count) % shapes.Count;
        var to = (from + 1) % shapes.Count;
        var amount = Math.Clamp(fraction - index, 0, 1);
        var result = new Point[PointCount];
        for (var pointIndex = 0; pointIndex < PointCount; pointIndex++)
        {
            var start = shapes[from][pointIndex];
            var end = shapes[to][pointIndex];
            result[pointIndex] = new Point(
    ...
    private static IReadOnlyList<Point[]> CreateShapes()
    {
        (double ViewBox, string Data)[] definitions =
        [
        ...
        var shapes = definitions
            .Select(definition => Resample(Normalize(Flatten(definition.Data)), PointCount))
            .ToList();
        shapes.Add(Enumerable.Range(0, PointCount)
            .Select(index =>
            {
                var angle = index / (double)PointCount * 2 * Math.PI;
                return new Point(Math.Cos(angle), 0.74 * Math.Sin(angle));
            })
            .ToArray());
        return shapes;
    }

    private static IReadOnlyList<Point> Flatten(string data)
    {
        var path = Geometry.Parse(data).GetFlattenedPathGeometry(0.15, ToleranceType.Absolute);
    ...
    private static Point[] Normalize(IReadOnlyList<Point> points)
    {
        var left = points.Min(point => point.X);
        var right = points.Max(point => point.X);
        var top = points.Min(point => point.Y);
        var bottom = points.Max(point => point.Y);
        var scale = Math.Min(2 / (right - left), 2 / (bottom - top));
        var centerX = (left + right) / 2;
        var centerY = (top + bottom) / 2;
        return points.Select(point => new Point(
            (point.X - centerX) * scale,
            (point.Y - centerY) * scale)).ToArray();
    }

    private static Point[] Resample(IReadOnlyList<Point> points, int count)
    {
        var closed = points.Concat([points[0]]).ToArray();
        var arcLengths = new double[closed.Length];
        for (var index = 1; index < closed.Length; index++)
            arcLengths[index] = arcLengths[index - 1] + (closed[index] - closed[index - 1]).Length;
        ...
        {
            var target = index / (double)count * totalLength;
            while (segmentIndex < arcLengths.Length - 1 && arcLengths[segmentIndex] < target)
                segmentIndex++;
            var segmentLength = arcLengths[segmentIndex] - arcLengths[segmentIndex - 1];
            var amount = segmentLength > 0
                ? (target - arcLengths[segmentIndex - 1]) / segmentLength
                : 0;
            result[index] = closed[segmentIndex - 1]
                + (closed[segmentIndex] - closed[segmentIndex - 1]) * amount;
        }
        return result;
    }

    private static Point ToCanvas(Point point, Point center, double scale) =>
